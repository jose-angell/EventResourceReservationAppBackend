...
namespace EventResourceReservationApp.UnitTests.Domain
{
    public class ResourceTests
    {
        ...
        public void Constructor_WithInvalidCategoryId_ShouldThrowArgumentException()
        {
            ...
            Guid createdByUserId = Guid.NewGuid();
            // Act & Assert
            var exception = Assert.Throws<ArgumentException>(() => new Resource(
                categoryId, name, description, quantity, price,
                authorizationType, locationId, createdByUserId));
        ...
        [Fact]
        public void Update_WithValidParameters_ShouldUpdateResource()
        {
            // Arrange
            var resource = new EventResourceReservationApp.Domain.Resource( 1, "Projector", "HD Projector", 5, 150.00m, ResourceAuthorizationType.Automatico, 2, Guid.NewGuid());
            int categoryId = 2;
            string name = "Projector update";
            string description = "HD + Projector";
            int quantity = 5;
            decimal price = 150.45m;
            ResourceAuthorizationType authorizationType = ResourceAuthorizationType.Manual;
            int locationId = 2;
            var estatusId = ResourceStatus.Eliminado;
            // Act
            resource.Update(categoryId, estatusId, name, description, quantity, price, authorizationType, locationId);
            // Assert
            Assert.Equal(categoryId, resource.CategoryId);
            Assert.Equal(name, resource.Name);
            Assert.Equal(description, resource.Description);
        ...
        [Fact]
        public void Update_WithInvalidCategoryId_ShouldThrowArgumentException()
        {
            var resource = new Resource(1, "Projector", "HD Projector", 5, 150.00m, ResourceAuthorizationType.Automatico, 2, Guid.NewGuid());
            int categoryId = 0;
            string name = "Projector update";
            string description = "HD + Projector";
            int quantity = 5;
            ...
            var estatusId = ResourceStatus.Eliminado;
            // Act && Assert            
            var exception = Assert.Throws<ArgumentException>(() => resource.Update(categoryId, estatusId, name, description, quantity, price, authorizationType, locationId));
            Assert.Equal("CategoryId cannot be empty. (Parameter 'categoryId')", exception.Message);
        }
        [Fact]
        public void Update_WithNegativeQuantity_ShouldThrowArgumentException()
        {
            // Arrange
            var resource = new Resource(1, "Projector", "HD Projector", 5, 150.00m, ResourceAuthorizationType.Automatico, 2, Guid.NewGuid());
            int categoryId = 1;
        ...
        [InlineData("")]
        [InlineData("   ")]
        public void Update_WithEmptyDescription_ShouldThrowArgumentException(string description)
        {
            // Arrange
            var resource = new Resource(1, "Projector", "HD Projector", 5, 150.00m, ResourceAuthorizationType.Automatico, 2, Guid.NewGuid());
            int categoryId = 1;
        ...
        [Fact]
        public void Update_WithEmptyPrice_ShouldThrowArgumentException()
        {
            // Arrange
            var resource = new Resource(1, "Projector", "HD Projector", 5, 150.00m, ResourceAuthorizationType.Automatico, 2, Guid.NewGuid());
            int categoryId = 1;
        ...
        [Fact]
        public void Update_WithEmptyAuthorizationType_ShouldThrowArgumentException()
        {
            // Arrange
            var resource = new Resource(1, "Projector", "HD Projector", 5, 150.00m, ResourceAuthorizationType.Automatico, 2, Guid.NewGuid());
            int categoryId = 1;
            ...
            int quantity = 5;
            decimal price = 150.45m;
            ResourceAuthorizationType authorizationType = (ResourceAuthorizationType)(-1);
            int locationId = 2;
            var estatusId = ResourceStatus.Eliminado;
            // Act && Assert            
            var exception = Assert.Throws<ArgumentException>(() => resource.Update(categoryId, estatusId, name, description, quantity, price, authorizationType, locationId));
        ...
        [Fact]
        public void Update_WithInvalidLocationId_ShouldThrowArgumentException()
        {
            // Arrange
            // Arrange
            var resource = new Resource(1, "Projector", "HD Projector", 5, 150.00m, ResourceAuthorizationType.Automatico, 2, Guid.NewGuid());
