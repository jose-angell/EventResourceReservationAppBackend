...
namespace EventResourceReservationApp.UnitTests.Application.UseCases.Reservations
{
    public class UpdateReservationUseCaseTests
    {
        ...
        private readonly UpdateReservationUseCase _useCase;
        private readonly Mock<ILogger<UpdateReservationUseCase>> _logger;

        public UpdateReservationUseCaseTests()
        {
            _unitOfWork = new Mock<IUnitOfWork>();
            _repository = new Mock<IReservationRepository>();
            _logger = new Mock<ILogger<UpdateReservationUseCase>>();
            _unitOfWork.Setup(u => u.Reservations).Returns(_repository.Object);
            _useCase = new UpdateReservationUseCase(_unitOfWork.Object, _logger.Object);
        }
        [Fact] 
        public async Task ExecuteAsync_WithValidRequest_ReturnsSuccess()
        {
            ...
            var updatedReservation = new UpdateReservationRequest
            {
                Id = reservationId,
                StartTime = DateTime.Now.AddHours(1),
                EndTime = DateTime.Now.AddHours(3),
                TotalAmount = 20m,
                ClientComment = "Updated comment",
        ...
        public async Task ExecuteAsync_WithInvalidRequest_ReturnsFailure()
        {
            ...
            Assert.Equal("TotalAmount must be a positive value. (Parameter 'totalAmount')", result.Message);
