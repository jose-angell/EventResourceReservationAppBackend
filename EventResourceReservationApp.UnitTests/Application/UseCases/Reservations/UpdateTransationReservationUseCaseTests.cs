...
{
    public class UpdateTransactionReservationUseCaseTests
    {
        private readonly Mock<IUnitOfWork> _unitOfWork;
        private readonly Mock<IReservationRepository> _repository;
        private readonly UpdateTransactionReservationUseCase _useCase;
        private readonly Mock<ILogger<UpdateTransactionReservationUseCase>> _logger;

        public UpdateTransactionReservationUseCaseTests()
        {
            _unitOfWork = new Mock<IUnitOfWork>();
            _repository = new Mock<IReservationRepository>();
            _logger = new Mock<ILogger<UpdateTransactionReservationUseCase>>();
            _unitOfWork.Setup(u => u.Reservations).Returns(_repository.Object);
            _useCase = new UpdateTransactionReservationUseCase(_unitOfWork.Object, _logger.Object);
        }
        [Fact]
        public async Task ExecuteAsync_WithValidRequest_ReturnsSuccess()
        {
            ...
            var updatedReservation = new UpdateTransactionReservationRequest
            {
                Id = reservationId,
                TrasnsationId = Guid.NewGuid(),
        ...
        public async Task ExecuteAsync_WithInvalidRequest_ReturnsFailure()
        {
            ...
            Assert.Equal("TransactionId cannot be empty. (Parameter 'transactionId')", result.Message);
